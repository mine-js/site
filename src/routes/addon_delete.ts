...
export default {
    method: 'get',
    route: '/addon/delete/:addonId/:addonCreator',
    execute: async (req: express.Request, res: express.Response, connection: mysql.Connection) => {
        ...
        if(rows.length > 0) {
            await connection.query('DELETE FROM addons WHERE id=?', [req.params.addonId])
            await connection.query('DELETE FROM releases WHERE id=?', [req.params.addonId])
